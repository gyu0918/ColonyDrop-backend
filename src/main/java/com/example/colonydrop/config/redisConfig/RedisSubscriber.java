...
public class RedisSubscriber implements MessageListener {
    ...
    public void onMessage(Message message, byte[] pattern) {
        try {
            ...
            // 채팅 채널 처리
            if (channel.startsWith(RedisPublisher.CHAT_CHANNEL_PREFIX)) {
                String roomType = channel.replace(RedisPublisher.CHAT_CHANNEL_PREFIX, "");
                ChatMessageDto chatMessage = objectMapper.readValue(body, ChatMessageDto.class);
                messagingTemplate.convertAndSend("/topic/chat/" + roomType, chatMessage);
                log.info("채팅 WebSocket 전송 완료 → roomType: {}", roomType);
                return;
            }
