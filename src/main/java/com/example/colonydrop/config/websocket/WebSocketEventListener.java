...
public class WebSocketEventListener {
    ...
    public void initUserCount() {
        stringRedisTemplate.opsForValue().set("site:users", "0");
        stringRedisTemplate.opsForValue().set("chat:users:gundam", "0");
        stringRedisTemplate.opsForValue().set("chat:users:openrun", "0");
        stringRedisTemplate.opsForValue().set("chat:users:sharing", "0");
        stringRedisTemplate.opsForValue().set("chat:users:free", "0");
        log.info("[WebSocketEventListener] 접속자 수 초기화 완료");
    }

    // webSocket 연결시
    @EventListener
    public void handleConnect(SessionConnectedEvent event) {
        Long count = stringRedisTemplate.opsForValue().increment("site:users");
        simpMessagingTemplate.convertAndSend("/topic/users", count);
        log.info("[WebSocketEventListener] 접속 - 현재 접속자 수: {}", count);
    }
